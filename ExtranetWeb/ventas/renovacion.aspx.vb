        ...
        Try
            If Not IsPostBack Then
                If Session("user") IsNot Nothing Then
                    metodos_master("Renovación de Servicios", 3, "Renovacion seleccion de productos")
                    mensajeria_error("", "")
                    Session("total_peb") = 0
                    Session("DTPorcentajeIva") = porcentajeIva() 'RenovacionAdapter.ConsultaPorcentajeIva()
                ...
                End If
            Else
                If Not Session("user") IsNot Nothing Then
                    Response.Redirect("./login.aspx", False)
                End If
            End If
    ...
    Private Sub btncontinuar_ServerClick(sender As Object, e As System.EventArgs) Handles btncontinuar.ServerClick
        ...
        If Not table Is Nothing Then
            If table.Rows.Count > 0 Then
                If CInt(Session("total_peb")) > 0 Then
                    mensajeria_error("error", "existen productos que no se puede aplicar débito recurrente.")
                Else
                    Response.Redirect("RenovacionFact.aspx", False)
                End If
            End If
        Else
            mensajeria_error("error", "Usted no ha seleccionado producto/vehículo.")
        End If
    End Sub
...
#Region "Procedimientos"
    ...
    Private Sub ConsultaProductoCliente()
        Try
            ...
            dTCstGeneral = RenovacionAdapter.ConsultaProductoClienteAux(Session("user_netsuite"), txtfiltrar.Text.Trim, "T")
    ...
    Private Sub CalculaItemsSeleccionados()
        Try
            ...
            Dim valueCellFechaIni As String
            Dim valueCellFechaFin As String
            Dim valueCellProductoDA As String
            Dim valueCellPromocionDA As String
            Dim valueCellItem As String
            Dim valueCellAnio As String
            Dim dTCstGeneral As New System.Data.DataSet
            Session("total_peb") = 0
            ...
            For Each item As RepeaterItem In Rpt_renovacion.Items
                ...
                Dim lblejecutiva As Label = CType(item.FindControl("lblgrupoitem1"), Label)
                Dim vehiculo_itm As String = lblgrupo.Text
                Dim vehiculo_nombre As String = lblvehiculonombre.Text
                Dim ejecutiva_itm As String = lblejecutiva.Text
                For Each sub_item As RepeaterItem In rptitm.Items
                    ...
                    If chkI.Checked Then
                        If cmbanio.SelectedValue > 0 Then
                            ...
                            dTCstPrecio = RenovacionAdapter.ConsultaPrecioProducto(vehiculo_itm, lblproducto.Text, cmbanio.SelectedValue, promocion, "NO", listado)
                            If dTCstPrecio.Tables.Count > 0 Then
                                If dTCstPrecio.Tables(1).Rows.Count > 0 Then
                                    For i As Integer = 0 To dTCstPrecio.Tables(1).Rows.Count - 1
                                        valuecontador += 1
                                        valueCellPrecioVenta = dTCstPrecio.Tables(1).Rows(i)("PRECIO_VENTA")
                                        valueCellPromocionCodigo = dTCstPrecio.Tables(1).Rows(i)("CODIGO_PROMOCION")
                                        valueCellPromocionValor = dTCstPrecio.Tables(1).Rows(i)("VALOR_PROMOCION")
                                        valueCellDescuentoValor = dTCstPrecio.Tables(1).Rows(i)("VALOR_DESCUENTO")
                                        valueCellPrecioTotal = dTCstPrecio.Tables(1).Rows(i)("PRECIO_TOTAL")
                                        valueCellIvaValor = dTCstPrecio.Tables(1).Rows(i)("IVA")
                                        valueCellPrecioCliente = dTCstPrecio.Tables(1).Rows(i)("PRECIO_CLIENTE")
                                        valueCellItem = dTCstPrecio.Tables(1).Rows(i)("ITEM")
                                        valueCellAnio = dTCstPrecio.Tables(1).Rows(i)("ANIO")
                                        valueCellPromocionDA = 0 'dTCstPrecio.Tables(0).Rows(0)("PROMOCION_DA")
                                        ...
                                        If (valueCellIvaValor > 0) Then
                                            ...
                                            Dim porcentajeIva As Decimal = Session("DTPorcentajeIva") / 100
                                        ...
                                        row("ANIO") = valueCellAnio
                                        row("FECHAFIN") = lblfechavencimiento.Text
                                        row("FECHAINIDA") = valueCellFechaIni
                                        row("FECHAFINDA") = valueCellFechaFin
                                        row("PRODUCTODA") = valueCellProductoDA
                                        row("IDITEM") = valueCellItem
                                        row("IDEJECUTIVO") = ejecutiva_itm
                                        table.Rows.Add(row)
                                    Next
                                End If
                            End If
